    ...
    if request.method == 'POST':
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(f"Unfortunately you don't have the sufficient amount of money to purchase {p_item_object.name}", category='danger')
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f'Congratulation! You have sold {s_item_object.name}!', category='success')
            else:
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        flash(f'Hi {user_to_create.username}, Welcome to Saar Market Place!', category='success')

        return redirect(url_for('market_page'))
    if form.errors != {}:
        for err in form.errors.values():
            flash(f'There was an error creating the user: "{err[0]}"', category='danger')
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and \
            ...
            flash(f'Hi {attempted_user.username}, You have logged in successfully.', category='success')
            return redirect(url_for('market_page'))
        else:
